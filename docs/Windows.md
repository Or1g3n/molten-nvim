...
> [!NOTE]
> Molten now uses a Lua-based architecture. **No `:UpdateRemotePlugins` required!**
> The issues described below with rplugin manifest generation are no longer relevant.

Molten should work on Windows with the standard installation process. If you encounter issues,
