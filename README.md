...
- **Programmatic access to kernel output** via callbacks and autocmds
...
| `MoltenEvaluateArgument`  | `[kernel] code`       | Evaluate given code in the given kernel. When kernel is omitted, auto-selects if only one kernel is active, otherwise prompts |
...
| `MoltenEnterOutput`       | none                  | Move into the active cell's output window.  Opens but does not enter the output if it's not open.  **must be called with `noautocmd`** (see [Keybindings](#keybindings)) |
...
| `MoltenOpenInBrowser`     | none                  | Open the current output in the browser.  **Currently this only supports cells with `'text/html'` outputs**, configured with `molten_auto_open_html_in_browser` |
| `MoltenImagePopup`        | none                  | Open an image from the current output with python's `Image.show()`. This will use your system's default image viewer |
...
| `MoltenLoad`              | `["shared"] [path]`   | Loads cell locations and output from a JSON file generated by `MoltenSave`. path functions the same as `MoltenSave`. If `shared` is specified, share the kernel with other buffers |
...
|----------------------                         |-------------------                                          |--------------------------------------------|
| `g:molten_auto_image_popup`                   | `true` \| (`false`)                                         | When true, cells that produce an image output will open the image output automatically |
| `g:molten_auto_init_behavior`                 | `"raise"` \| (`"init"`)                                     | When set to "raise" commands which would otherwise ask for a kernel when they're run will raise an error instead |
...
| `g:molten_image_location`                     | (`"both"`) \| `"float"` \| `"virt"` \|                      | Where images will be displayed, either the floating window only, virtual text output only, or both |
| `g:molten_image_provider`                     | (`"none"`) \| `"image.nvim"` \| `"wezterm"` \|              | How images are displayed see [Images](#images) for more details |
| `g:molten_open_cmd`                           | (`nil`) \| Any command                                      | Defaults to `xdg-open` on Linux, `open` on Darwin, and `start` on Windows. But you can set it to any command |
...
| `g:molten_output_show_more`                   | `true` \| (`false`)                                         | When the window can't display the entire contents of the output buffer, shows the number of extra lines |
| `g:molten_output_virt_lines`                  | `true` \| (`false`)                                         | Pad the main buffer with virtual lines so the floating window doesn't cover anything |
| `g:molten_output_win_border`                  | (`{ "", "━", "", "" }`) \| any value for `border` in `:h nvim_open_win()`| Some border features will not work if you don't specify your border as a table |
| `g:molten_output_win_cover_gutter`            | (`true`) \| `false`                                         | Should the output window cover the gutter (numbers and sign col), or not |
| `g:molten_output_win_hide_on_leave`           | (`true`) \| `false`                                         | After leaving the output window (via `:q` or switching windows), do not attempt to redraw |
...
| `g:molten_tick_rate`                          | (`500`) \| int                                              | How often (in ms) we poll the kernel for updates.  Determines how quickly the ui will update |
| `g:molten_use_border_highlights`              | `true` \| (`false`)                                         | When true, uses different highlights for output border depending on the state of the cell |
| `g:molten_limit_output_chars`                 | (`1000000`) \| int                                          | Limit on the number of chars in an output.  If you're lagging your editor with too much output, lower this |
| `g:molten_virt_lines_off_by_1`                | `true` \| (`false`)                                         | Allows the output window to cover exactly one line of the regular buffer when `output_virt_lines` is true |
| `g:molten_virt_text_output`                   | `true` \| (`false`)                                         | When true, show output as virtual text below the cell, virtual text stays after leaving the cell |
| `g:molten_virt_text_max_lines`                | (`12`) \| int                                               | Max height of the virtual text |
| `g:molten_wrap_output`                        | `true` \| (`false`)                                         | Wrap output text |
| [DEBUG] `g:molten_show_mimetype_debug`        | `true` \| (`false`)                                         | Before any non-iostream output chunk, the mime-type for that output chunk is shown |
...
- `MoltenOutputDone`: runs when any cell execution completes. `data` field contains execution results
...
For `MoltenOutputDone` you can access the execution results: 

```lua
vim.api.nvim_create_autocmd("User", {
  pattern = "MoltenOutputDone",
  callback = function(args)
    -- args.data. kernel_id:  string - the kernel that ran the code
    -- args.data.output:  string - plain text output
    -- args.data.success: boolean - whether execution succeeded
    -- args.data. execution_count: number - the execution count
    print("Execution " .. args.data.execution_count .. " completed")
  end
})
```

**Example:  Log all outputs to a results buffer**
...
vim.api.nvim_create_autocmd("User", {
  pattern = "MoltenOutputDone",
  callback = function(args)
    local lines = vim.split(args.data.output, "\n")
    table.insert(lines, 1, string.format("--- [%d] %s ---",
      args.data.execution_count,
      args.data.success and "✓" or "✗"
    ))
    vim.api.nvim_buf_set_lines(results_buf, -1, -1, false, lines)
  end
})
```
...
<details>
  <summary>MoltenEvaluateArgument (Function Form)</summary>

When called as a function, `MoltenEvaluateArgument` supports an options table with an `on_done`
callback for programmatic access to kernel output.

**Basic usage (auto-select kernel):**
...
**Example: Insert output at cursor position**
...
**Example: Programmatic data extraction**
...
print(json.dumps(result))
  ...
  on_done = function(result)
    local data = vim.json.decode(result.output)
    vim.notify("DataFrame has " .. data.count .. " rows")
  end
})
```
...
<details>
  <summary>MoltenGetOutput</summary>

`MoltenGetOutput()` - retrieves output from the current cell (the cell under the cursor).

Returns a table with the following fields: 
- `status`: string - `"done"`, `"running"`, or `"hold"`
- `success`: boolean - whether the execution succeeded
- `output`: string - plain text output
- `execution_count`: number - the execution count

```lua
local output = vim.fn.MoltenGetOutput()
if output then
  print("Status: " ..  output.status)
  print("Success: " .. tostring(output.success))
  print("Output: " .. output. output)
  print("Execution count: " .. output.execution_count)
end
```

Returns `nil` if there is no active cell under the cursor. 
